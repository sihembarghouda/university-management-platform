...
// Absence microservice base URL (extracted service runs on port 3003)
...
const AbsenceManagement = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // If user is an etudiant, request only their absences
      try {
        const tokenUser = localStorage.getItem('user');
        const parsed = tokenUser ? JSON.parse(tokenUser) : null;
        if (parsed && parsed.role === 'etudiant') {
          const response = await axios.get(`${API_URL}/absences/etudiant/${parsed.id}`);
